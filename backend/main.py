...
def init_data():
    ...
    try:
        if db.query(User).count() == 0:
            ...
            db.add(user)
            db.flush()
            target = UserTarget(user_id=user.id, calories=2000, protein=50,
                                fat=65, carbs=250)
            db.add(target)
            db.commit()
        if db.query(FoodItem).count() == 0:
            foods = [
                FoodItem(name="Egg", calories_per_100g=155,
                         protein_per_100g=13, fat_per_100g=11,
                         carbs_per_100g=1.1),
                FoodItem(name="Chicken Breast", calories_per_100g=165,
                         protein_per_100g=31, fat_per_100g=3.6,
                         carbs_per_100g=0),
                FoodItem(name="White Rice (cooked)", calories_per_100g=130,
                         protein_per_100g=2.7, fat_per_100g=0.3,
                         carbs_per_100g=28),
                FoodItem(name="Olive Oil", calories_per_100g=884,
                         protein_per_100g=0, fat_per_100g=100,
                         carbs_per_100g=0),
                FoodItem(name="Banana", calories_per_100g=89,
                         protein_per_100g=1.1, fat_per_100g=0.3,
                         carbs_per_100g=23),
                FoodItem(name="Milk (whole)", calories_per_100g=61,
                         protein_per_100g=3.2, fat_per_100g=3.3,
                         carbs_per_100g=4.8),
                FoodItem(name="Bread (white)", calories_per_100g=265,
                         protein_per_100g=9, fat_per_100g=3.2,
                         carbs_per_100g=49),
                FoodItem(name="Salmon", calories_per_100g=208,
                         protein_per_100g=20, fat_per_100g=13,
                         carbs_per_100g=0),
            ]
            db.add_all(foods)
            db.commit()
...
app.mount("/", StaticFiles(directory=os.path.dirname(__file__),
