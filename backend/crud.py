...
def create_meal(db: Session, meal_data: MealCreate, user_id: int = 1):
    ...
    """
    meal = Meal(date=meal_data.date, meal_type=meal_data.name,
                user_id=user_id)
    db.add(meal)
    db.flush()  # populate meal.id after insert
    entry = MealEntry(meal_id=meal.id, food_id=meal_data.food_id,
                      quantity_grams=meal_data.quantity)
    db.add(entry)
    db.commit()
    db.refresh(meal)
    return meal
...
def get_meals_by_day(db: Session, day: date, user_id: int = 1):
    ...
    """
    meals = db.query(Meal).filter(Meal.date == day,
                                  Meal.user_id == user_id).all()
    total = {"calories": 0, "protein": 0, "fat": 0, "carbs": 0}
    meal_list = []
    for meal in meals:
        for entry in meal.entries:
            food = entry.food
            # Convert the per-100g nutrition values to the actual serving
            # size.  `quantity_grams` is the recorded serving size in
            # grams; dividing by 100 gives the multiplier to apply to the
            # per-100g nutrition fields.
            q = entry.quantity_grams / 100
            # Multiply per-100g values by the multiplier to get per-serving
            # values.
...
def delete_meal(db: Session, meal_id: int, user_id: int = 1):
    ...
    meal = db.query(Meal).filter(Meal.id == meal_id,
...
def get_week_stats(db: Session, week_start: date, user_id: int = 1):
    ...
    for i in range(7):
        ...
        # Round aggregated totals to one decimal place for consistent UI
        # display and to avoid floating point noise in charts or reports.
    ...
    return WeekSummary(week_start=week_start, week_end=week_end,
